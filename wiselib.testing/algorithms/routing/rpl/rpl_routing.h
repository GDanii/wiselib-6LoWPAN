...
#define DIO_MESSAGE_LENGTH 54 //length in octets of the IPv6 message payload
...
namespace wiselib
{
	...
	*/
	...
	public:
		...
		typedef MapStaticVector<OsModel, node_id_t, uint8_t, 20> NeighborSet; //uint8_t represent the bidirectionality
		...
		//Routing Table.... for now it is supported only multipoint-to-point traffic...
		//.... so nodes have only the entry ( dodag_root, next_hop ) ... 
		//.... TO UPDATE the algorithm in order to support other kind of traffic flows
		typedef MapStaticVector<OsModel , node_id_t, node_id_t, 10> RoutingTable;
		typedef wiselib::pair<node_id_t, node_id_t> rt_pair_t;
		typedef typename wiselib::MapStaticVector<OsModel , node_id_t, node_id_t, 10>::iterator RoutingTable_iterator;
		...
		*/
		...
		enum ObjectiveFunctionTypes
		{
			...
			OF_HOP_COUNT = 1,  //HOP COUNT CONSTRAINT (For Parent Selection) & METRIC (For the computation of the preferred parent)
			OF2 = 2,
			OF3 = 3
		};
		...
		*/
		...
		// N.B. TO UPDATE IN THE IPv6 CLASS... ASK DANIEL or PULL A REQUEST ON GITHUB.........................
		/**
		...
		*/
		...
		uint8_t scan_neighbors( node_id_t from );

		//void start2( void *userdata );

		//an external application should be able ONLY to read the routing table!!!! TO CHANGE!
		// redefine methods 'find', get, etc....
	...
	private:
		...
		*/
		...
		uint32_t max_interval_; //depends on i_min and i_max
	...
	RPLRouting()
		: rpl_instance_id_ (1),   //suppose now there's only one instance_id (To be updated)
		state_ (Unconnected),
		dio_count_ (0),
	...
	set_dodag_root( bool root, uint16_t ocp = 0 )
	{
		...
		if ( root )
		{
			...
			//Don't need them for the moment
	...
	start( void )
	{
		...
		timer().template set_timer<self_type, &self_type::start2>( 3500, this, 0 );
	...
	int
	RPLRouting<OsModel_P, Radio_IP_P, Radio_P, Debug_P, Timer_P, Clock_P>::
	send_dio( node_id_t destination, uint16_t len, block_data_t *data )   //to understand better how to use it, even for data messages
	{

		
		dio_message_->set_next_header(Radio_IP::ICMPV6);
	...
	receive( node_id_t from, size_t packet_number, block_data_t *data )
	{
		...
		//packet_number is the second parameter of the receive function... change the function and understand!
		...
		/*//NB: TO SOLVE AND THEN UNCOMMENT... the lower layers don´t compute the checksum upon sending!!!!!
		if( checksum != message->generate_checksum() )
		{
			
			#ifdef ROUTING_RPL_DEBUG
			//debug().debug( "RPL: Dropped packet (checksum error), in packet: %x computed: %x\n", checksum, message->generate_checksum() );
			
			debug().debug( "RPL Routing: Checksum error, in packet: %x computed: %x\n", checksum, message->generate_checksum() );
			#endif
		...
		*/
		...
		//What about data messages that contain a RPL Option????????? SEE LATER
		...
		if( mop_check == 0 && (typecode != DODAG_INF_OBJECT && typecode != DODAG_INF_SOLICIT) )
		{
			packet_pool_mgr_->clean_packet( message );
			return;
		}
	
		if( ( mop_check == 1 || mop_check == 2 ) && (typecode != DODAG_INF_OBJECT && typecode != DODAG_INF_SOLICIT &&
		...
		if ( typecode == DODAG_INF_OBJECT )
		{
			...
			{
				//Compare the received message with the memorized one..
				...
				else
				{
					...
					//the node need to have a rank for each parent in the parent's set

					//process only messages sent by nodes whose rank is lower than the rank of the actual node
					if (parent_rank < rank_ )  //to add the equality???
					{
						if( scan_neighbors( sender ) == 0 )
						{
						...
						{	
							...
							routing_table_.erase( dodag_id_ );
							routing_table_.insert( rt_pair_t( dodag_id_, preferred_parent_ ) );
						}
		...
		{
			//IF MOP = 1 and I'm not the root send directly the msg to the next hop toward the root
	...
	first_dio( node_id_t from, block_data_t *data, uint16_t length )
	{
		...
		uint8_t ret = options_check( data, length_checked, length, from );
		...
		//THE MESSAGE LENGTH HAS TO BE SET HERE
	...
	{		
		...
		//1(grounded) 0(predefined) 000(MOP: no downward routes) 000(default prf) = 2^7 = 128
	...
	dio_packet_initialization( uint8_t position, bool grounded )
	{
		...
		if ( grounded )//1(grounded) 0(predefined) 000(no downward routes) 000(default prf) = 2^7 = 128
			setter_byte = 128;
		else
			setter_byte = 0;
	...
	scan_neighbors( node_id_t from )
	{
		...
		if (it == neighbor_set_.end())
		{
			...
			//IDEA: check neighbors each time the timer elapses!!!!!!!
